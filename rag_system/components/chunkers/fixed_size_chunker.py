...
class FixedSizePDFChunker(ChunkerBase):
    ...
    def chunk_helper(self, data: List[DataChunk]):
        '''
        input: list of DataChunk (chunk by page)
        output: list of DataChunk (chunk by section)
        '''
        SENTENCE_ENDINGS = [".", "!", "?"]
        WORDS_BREAKS = [",", ";", ":", " ", "(", ")", "[", "]", "{", "}", "\t", "\n"]
    ...
    def chunk(self, data: List[DataChunk]) -> List[DataChunk]:
        '''
        input: list of DataChunk (chunk by page)
        output: list of DataChunk (chunk by section), with chunk_id
        '''
        source_url = data[0].source.split("/")[-1] 
        print("source_url", source_url)
        chunked_data = []

        for i, (section_text, page_num, offset) in enumerate(self.chunk_helper(data)):
